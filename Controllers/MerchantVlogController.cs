...
namespace backend.Controllers
{
    ...
    /// 前端再輪詢 Status 直到影片完成（完成時才寫入 merchant_media）。
    /// </summary>
    [ApiController]
    ...
    public class MerchantVlogController : ControllerBase
    {
        ...
        /// （因 merchant_media.au_id 為 not null，需在完成時綁定操作此功能的帳號）。
        /// </summary>
        [HttpPost]
        [Route("CreateFinal")]
        [Authorize]
        ...
        /// <summary>
        /// 依 task_id 查詢影片合成進度。**必須帶 Token**。查到 ready 時自動寫入 merchant_media。
        /// </summary>
        [HttpGet]
        [Route("Status/{taskId}")]
        [Authorize]
        public async Task<IActionResult> Status(string taskId)
        {
            try
            {
                var result = await _service.CheckStatusAsync(taskId, User.GetAuId());
